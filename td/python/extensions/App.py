...
class App:
		...
		def loadEnvVars(self):
				...
				envFilePath = os.path.join(
						project.folder, 'data', '.env')  # path to the .env file
		...
		def loadPythonModules(self):
				...
				config.AddPyDirToPath(os.path.join(
						project.folder, 'python', '_local_modules'))
		...
		def LaunchPlaylist(self, delayFrames=0):
				...
				if op.AppStore.GetBoolean('is_production') == True:
						run(f"op('{self.ownerComp.path}').LaunchOutputWindow()",
								delayFrames=1000)
		...
		def TriggerLaunch(self, delayFrames=0):
				...
				run(f"op('{self.ownerComp.path}').PulseTriggerLaunch()",
		...
		def LaunchOutputWindow(self):
				window1 = op('window1')
				# window1.par.performance.pulse()
				window1.par.winopen.pulse()
		...
		def AddStoreListeners(self):
				...
				op.AppStore.AddListener(self, App.EVENT_PLAYLIST_NEXT)  # triggered by players
