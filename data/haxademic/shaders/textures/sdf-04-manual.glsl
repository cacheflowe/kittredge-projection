...
uniform vec3 cubeSize = vec3(0.6, 0.6, 0.6);
uniform vec3 rotation = vec3(0., 0., 0.);
uniform vec3 cubePos = vec3(0., 0., -3.);
...
void main( void ) {
  ...
  float timeSlow = time/1.;
    ...
    // float temp = udBox( ipRotated, vec3(cubeSize) );
    float temp = udBoxTwisted( ipRotated, cubeSize, twistAmp );
